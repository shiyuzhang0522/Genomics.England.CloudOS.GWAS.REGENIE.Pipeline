...
EOF
    ...
    echo
    echo "Matching REGENIE files:"
    ls -lah chr${chr}.GEL_CM_REGENIE_step2* || true

    echo
    echo "All regular files in PWD:"
    ...
    if [[ -s "\${GWAS_FILE}" ]]; then
        echo "GWAS_OUTPUT_STATUS=FOUND_NONEMPTY"
        ls -lh "\${GWAS_FILE}"
    else
        echo "GWAS_OUTPUT_STATUS=MISSING_OR_EMPTY"
    fi

    if [[ -s "\${LOG_FILE}" ]]; then
        echo "REGENIE_LOG_STATUS=FOUND_NONEMPTY"
        ls -lh "\${LOG_FILE}"
    else
        echo "REGENIE_LOG_STATUS=MISSING_OR_EMPTY"
    fi
