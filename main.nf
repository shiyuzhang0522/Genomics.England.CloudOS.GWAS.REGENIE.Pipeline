...
/*
===============================================================================
GEL CloudOS | REGENIE Step 2 rare protein-coding association analysis
Author: Shelley

Purpose
-------
Reuse the working GEL GWAS PGEN/phenotype/covariate/LOCO framework while
preserving the supplied UKBB rare-coding mask and association settings.

Default design
--------------
22 autosomes x 4 masks x 3 AAF thresholds = 264 independent association tasks.
...
UKBB's documented defaults are explicit: --minMAC 5 --build-mask max
--vc-MACthr 10. Other REGENIE defaults are retained. In particular, the
...
The precomputed chr16 masks already exclude ENSG00000310590; this workflow
performs no additional gene exclusions and does not rebuild annotations.

Outputs
-------
UKBB-style <outdir>/<mask>/AAF_<AAF>/chrN.<mask>.AAF_<AAF>* filenames.
...
The QC TSV records result rows and represented genes; it does not certify
...
Pilot: --chromosomes 21 --masks pLoF_only --aaf_thresholds 0.01
...
process REGENIE_RARE_CODING {
    ...
    tuple val(chr),
          ...
          val(mask),
          val(aaf),
          path(annotation, name: 'coding.annotation.txt'),
          path(setlist, name: 'coding.setlist.txt'),
          path(maskdef, name: 'coding.maskdef.txt')
    ...
    tuple val(chr), val(mask), val(aaf),
          path("chr${chr}.${mask}.AAF_${aaf}_CM.regenie"), emit: results

    tuple val(chr), val(mask), val(aaf),
          path("chr${chr}.${mask}.AAF_${aaf}_masks.snplist"), emit: mask_snplists

    tuple val(chr), val(mask), val(aaf),
          path("chr${chr}.${mask}.AAF_${aaf}.qc.tsv"), emit: qc_summaries

    tuple val(chr), val(mask), val(aaf),
          path("chr${chr}.${mask}.AAF_${aaf}.log"),
          path("chr${chr}.${mask}.AAF_${aaf}.console.log"),
          path("chr${chr}.${mask}.AAF_${aaf}.command.sh"),
          path("chr${chr}.${mask}.AAF_${aaf}.pred.list"), emit: logs

    // The report may be absent when REGENIE finds nothing to report.
    tuple val(chr), val(mask), val(aaf),
          path("chr${chr}.${mask}.AAF_${aaf}_masks_report.txt"),
          optional: true, emit: burden_reports

    script:

    def prefix = "chr${chr}.${mask}.AAF_${aaf}"
    ...
    echo "============================================================"
    echo "GEL REGENIE rare-coding association: ${prefix}"
    ...
    for input_file in dragen.pgen dragen.pvar dragen.psam variant_ids.txt \\
        ...
        GEL_CM_REGENIE_step1_1.loco coding.annotation.txt \\
        coding.setlist.txt coding.maskdef.txt
    do
    ...
    awk -v mask='${mask}' 'NF!=2 || \$1!=mask || \$2!=mask {bad=1}
         END {exit (bad || NR!=1)}' coding.maskdef.txt || {
        echo '[ERROR] Mask definition does not match the selected mask.' >&2; exit 1;
    }

    awk -v mask='${mask}' 'NF!=3 || \$3!=mask {exit 1}' coding.annotation.txt || {
    ...
    wc -l variant_ids.txt gnomad_exclude.txt coding.annotation.txt coding.setlist.txt
    cat coding.maskdef.txt
    ...
    # ------------------------------------------------------------------------
    # 3. Preserve the UKBB rare-coding analysis settings
    # ------------------------------------------------------------------------
    ...
    regenie \\
        ...
        --anno-file coding.annotation.txt \\
        --set-list coding.setlist.txt \\
        --mask-def coding.maskdef.txt \\
        --aaf-bins ${aaf} \\
        --vc-maxAAF ${aaf} \\
    ...
    # ------------------------------------------------------------------------
    # 4. Require REGENIE outputs and summarize represented result genes
    # ------------------------------------------------------------------------
    ...
    # count records, distinct gene IDs, and missing LOG10P values by TEST.
    awk -v chr='${chr}' -v mask='${mask}' -v aaf='${aaf}' '
        ...
        NF {
            ...
            test=\$(col["TEST"]); id=\$(col["ID"]); split(id,parts,"."); gene=parts[1]
            rows[test]++; total++
...
'''
    ...
    if (masks.any { !(it in ['pLoF_only', 'pLoF_Dmis', 'Missense', 'Synonymous']) } ||
        masks.unique(false).size() != masks.size()) {
        error 'Masks must be unique selections from pLoF_only,pLoF_Dmis,Missense,Synonymous.'
    }
    ...
    REGENIE_RARE_CODING(task_ch, phenotype_ch, covariates_ch, loco_ch)
}
